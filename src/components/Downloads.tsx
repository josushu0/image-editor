		...
		<div className="flex flex-col gap-4">
			...
			<div className="flex flex-col">
				<div className={`flex border-2 border-neutral-900 transition-all ${dropdown ? 'rounded-none' : 'rounded-xl'}`}>
					<button onClick={() => handleDownload()} disabled={!image.src} className="p-2 border-r-2 border-neutral-900 grow rounded-l-xl hover:bg-neutral-200 disabled:bg-neutral-200">{downloadOption}</button>
					<button onClick={() => setDropdown(!dropdown)} className="flex justify-center items-center p-2 rounded-r-xl hover:bg-neutral-200">
						<ChevronDownIcon className="h-6" />
					</button>
				</div>
				{dropdown &&
					<>
						<button onClick={() => handleSelection('PNG')} className="p-2 border-2 border-t-0 border-neutral-900 hover:bg-neutral-200">PNG</button>
						<button onClick={() => handleSelection('JPG')} className="p-2 border-2 border-t-0 border-neutral-900 hover:bg-neutral-200">JPG</button>
					</>
				}
			</div>
		</div>
